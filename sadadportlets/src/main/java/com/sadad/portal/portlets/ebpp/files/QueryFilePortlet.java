package com.sadad.portal.portlets.ebpp.files;
...
import com.sadad.portal.helper.EbppFilesHelper;

public class QueryFilePortlet extends SadadGenericPortlet
{
	private static final String CLASS_NAME = QueryFilePortlet.class.getName();
	private static final Logger logger = Logger.getLogger(CLASS_NAME);
	
	private static final String JSP_FOLDER = "/jsp/html/ebpp/files/"; // JSP folder name
	private static final String VIEW_JSP_INDEX = "QueryFileIndex"; // JSP file name to be rendered on the view mode	
	private static final String VIEW_JSP_QUERY_FILE_FORM = "QueryFileForm"; // JSP file name to be rendered on the view mode

	private EbppFilesHelper hlpObj = new EbppFilesHelper();
	private EbppFilesSessionDataBean sesObj;
	...
	public void doView(RenderRequest request, RenderResponse response) throws PortletException, IOException
	{
		...
		sesObj = getSessionBeanObject(request, EbppFilesSessionDataBean.class);
		if (sesObj == null)
		{
			sesObj = new EbppFilesSessionDataBean();
			sesObj.getScreen().setContainer1(getJspFilePath(JSP_FOLDER, VIEW_JSP_QUERY_FILE_FORM));
			sesObj.getScreen().setContainer2(null);
			sesObj = populateUserDetailsInSessionBean(request, sesObj, EbppFilesSessionDataBean.class);
			sesObj = grabLtpaToken(request, sesObj, EbppFilesSessionDataBean.class);
		}
	...
	public void serveResource(ResourceRequest request, ResourceResponse response) throws PortletException, IOException
	{
		...
		sesObj = getSessionBeanObject(request, EbppFilesSessionDataBean.class);
		sesObj = populateRequestParamsInSessionBean(request, sesObj, EbppFilesSessionDataBean.class);
		sesObj = callRequiredBackEndServiceOperation(sesObj, hlpObj, EbppFilesSessionDataBean.class);
