package com.sadad.portal.portlets.rules.config;
...
import com.sadad.portal.helper.BusinessRulesHelper;

public class RulesConfigurationPortlet extends SadadGenericPortlet
{
	private static final String CLASS_NAME = RulesConfigurationPortlet.class.getName();
	private static final Logger logger = Logger.getLogger(CLASS_NAME);
	
	private static final String JSP_FOLDER = "/jsp/html/rules/config/"; // JSP folder name
	private static final String VIEW_JSP_INDEX = "RulesConfigurationIndex"; // JSP file name to be rendered on the view mode
	private static final String VIEW_JSP_RULES_CONFIGURATION_FORM = "RulesConfigurationForm";

	private BusinessRulesHelper hlpObj = new BusinessRulesHelper();
	private BusinessRulesSessionDataBean sesObj;
	...
	public void doView(RenderRequest request, RenderResponse response) throws PortletException, IOException
	{
		...
		sesObj = getSessionBeanObject(request, BusinessRulesSessionDataBean.class);
		if (sesObj == null)
		{
			sesObj = new BusinessRulesSessionDataBean();
			sesObj.getScreen().setContainer1(getJspFilePath(JSP_FOLDER, VIEW_JSP_RULES_CONFIGURATION_FORM));
			sesObj.getScreen().setContainer2(null);
			sesObj = populateUserDetailsInSessionBean(request, sesObj, BusinessRulesSessionDataBean.class);
		}
	...
	public void serveResource(ResourceRequest request, ResourceResponse response) throws PortletException, IOException
	{
		...
		sesObj = getSessionBeanObject(request, BusinessRulesSessionDataBean.class);
		sesObj = populateRequestParamsInSessionBean(request, sesObj, BusinessRulesSessionDataBean.class);
		sesObj = callRequiredBackEndServiceOperation(sesObj, hlpObj, BusinessRulesSessionDataBean.class);
