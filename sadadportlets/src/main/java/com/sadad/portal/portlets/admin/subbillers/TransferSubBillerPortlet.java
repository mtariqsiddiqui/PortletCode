package com.sadad.portal.portlets.admin.subbillers;
...
import com.sadad.portal.helper.SubBillerHelper;

public class TransferSubBillerPortlet extends SadadGenericPortlet
{
	private static final String CLASS_NAME = TransferSubBillerPortlet.class.getName();
	private static final Logger logger = Logger.getLogger(CLASS_NAME);
	
	private static final String JSP_FOLDER = "/jsp/html/admin/subbillers/"; // JSP folder name
	private static final String VIEW_JSP_INDEX = "SubBillerIndex";
	private static final String VIEW_JSP_TRANSFER_SUBBILLER_FORM = "TransferSubBillerForm"; // JSP file name to be rendered on the view mode
	
	private SubBillerHelper hlpObj = new SubBillerHelper();
	private SubBillerSessionDataBean sesObj;
	...
	public void doView(RenderRequest request, RenderResponse response) throws PortletException, IOException
	{
		...
		sesObj = getSessionBeanObject(request, SubBillerSessionDataBean.class);
		if (sesObj == null)
		{
			sesObj = new SubBillerSessionDataBean();
			sesObj.getScreen().setContainer1(getJspFilePath(JSP_FOLDER, VIEW_JSP_TRANSFER_SUBBILLER_FORM));
			sesObj.getScreen().setContainer2(null);
			sesObj = populateUserDetailsInSessionBean(request, sesObj, SubBillerSessionDataBean.class);
		}
	...
	public void serveResource(ResourceRequest request, ResourceResponse response) throws PortletException, IOException
	{
		...
		sesObj = getSessionBeanObject(request, SubBillerSessionDataBean.class);
		sesObj = populateRequestParamsInSessionBean(request, sesObj, SubBillerSessionDataBean.class);
		sesObj = callRequiredBackEndServiceOperation(sesObj, hlpObj, SubBillerSessionDataBean.class);
