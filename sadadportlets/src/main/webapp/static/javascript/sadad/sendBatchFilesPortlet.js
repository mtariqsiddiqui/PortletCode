...
function onContainer2LoadsFromModule() {
	setTimeout(function(){ makePageBusy(); $('#detailTable').DataTable().destroy(); }, 1000);
	setTimeout(function(){ 
		var _batTbl = $('#detailTable').DataTable( { dom : 'lfrtBip', language: { "url": "/static/javascript/library/i18n/"+_objKV['pageLanguage']+".json" },
			buttons : [ 'copyHtml5', 'csvHtml5', 'excelHtml5', 'pdfHtml5', 'print', {
				text: _objKV['send-all-files'], 
				action: function () {
					let ct = _batTbl.rows().count();
					let dt = _batTbl.rows();
					for(let i=0; i < ct; i++) {
						sendBatchFileMessage(document.getElementById('txt_SBF_' + dt.data()[i][4]).value, 1);
					}
				}
			}], 
		});
...
function setDatesForSubmission(doQueryFormSubmission) {
		let dateRangeFrom = new Date($('#txtRangeFromDate').val()); // Form input calendar field
		let dateRangeTo = new Date($('#txtRangeToDate').val()); // Form input calendar field
		...
		$('#txtDateRangeFrom').val(formatDateYMD(dateRangeFrom)); // Form parameter hidden field
		$('#txtDateRangeTo').val(formatDateYMD(dateRangeTo)); // Form parameter hidden field
 ...
 */
function sendBatchFileMessage(pUrl) {
    makePageBusy();
