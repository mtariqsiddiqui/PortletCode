...
<form name="frmSendBatchFile" id="frmSendBatchFile" method="post" action="<portlet:resourceURL id="RegenerateBatchFilesSummary" />">
	...
	<tbody>
		...
		<tr>
			<td>
				<fieldset>
					<legend>
						<fmt:message key="ow.portlet.label.regenerate-batch-files" bundle="${bndlLang}" />
					</legend>
					<table style="width: 100%;">
						<tr>
							<td>
												...
												<c:if test="${psb.partnerType == 'biller' or psb.partnerType == 'sadad'}">
													<select  class="outputData" id="cmbBillerId" style="display: inline;" onchange="partnerKeyChange(this.value);">
														<c:choose>
															...
															<c:otherwise>
																<option value=""><fmt:message key="ow.portlet.label.please-select" bundle="${bndlLang}"/></option>
											...
											<td class="outputDataCell" width="100%" valign="top" nowrap>
												<label class="label" for="txtRangeFromDate"><fmt:message key="ow.portlet.label.from" bundle="${bndlLang}" />
													<input class="outputData readonly" value="${psb.dateRangeFrom}" id="txtRangeFromDate" 
														autocomplete="off" maxlength="10" type="text" required placeholder="YYYY-MM-DD" onchange="updateDateSelection();"/>
												</label>
						...
						<tr>
							<td>
								...
								<portlet:resourceURL id="RegenerateBatchFilesQueryForm" var="clearURL"><portlet:param name="param_operation" value="clearSessionBeanObject"/></portlet:resourceURL>
