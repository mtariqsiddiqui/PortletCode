	...
	<tr>
		...
		<td>
			<c:if test="${rule.value.configType eq 'ATTRIBUTE Based'}">
				<portlet:resourceURL var="ruleDetailsUrl" id="RulesConfigurationAttributeDetails">
			...
			<c:if test="${rule.value.configType eq 'TEMPLATE Based'}">
				<c:if test="${not empty psb.partnerConfiguration[rule.key]}">
					<portlet:resourceURL var="ruleDetailsUrl" id="RulesConfigurationTemplateDetails">
						<portlet:param name="param_configrationKey" value="${rule.key}"/>
					</portlet:resourceURL>
				</c:if>				
				<c:if test="${empty psb.partnerConfiguration[rule.key]}">
					<portlet:resourceURL var="ruleDetailsUrl" id="RulesConfigurationTemplateDetails">
						...
						<portlet:param name="param_operation" value="callGetPartnerConfiguration"/>
						<portlet:param name="param_clearSecondContainer" value="false"/>
					</portlet:resourceURL>
				</c:if>
			</c:if>
