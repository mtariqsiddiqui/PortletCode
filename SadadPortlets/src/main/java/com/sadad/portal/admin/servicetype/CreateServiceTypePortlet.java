...
public class CreateServiceTypePortlet extends SadadGenericPortlet
{
	...
	private ServiceTypeSessionBean acSesObj;
	/**
	...
	public void doView(RenderRequest request, RenderResponse response) throws PortletException, IOException
	{
		...
		acSesObj = (ServiceTypeSessionBean) request.getPortletSession().getAttribute(PortalConstant.PORTLET_SESSION_BEAN, PortletSession.PORTLET_SCOPE);
		if(acSesObj == null)
		{
			acSesObj = ach.getServiceTypeObject();
			acSesObj.getScreen().setContainer1(getJspFilePath(JSP_FOLDER, VIEW_JSP_CREATE_SERVICE_TYPE_FORM));
...
//		SadadPortalUtils.logSessionAttributesWithValues(request, methodName, logger);
	...
	public void serveResource(ResourceRequest request, ResourceResponse response) throws PortletException, java.io.IOException
	{
		...
		acSesObj = (ServiceTypeSessionBean) request.getPortletSession().getAttribute(PortalConstant.PORTLET_SESSION_BEAN, PortletSession.PORTLET_SCOPE);
		ach.setServiceTypeObject(acSesObj);

		if(jspResourceId.equals(VIEW_JSP_CREATE_SERVICE_TYPE_FORM))
		{
			ach.callCreateServiceType(request.getParameter("txtServiceType"), request.getParameter("txtDescription"));
		}
