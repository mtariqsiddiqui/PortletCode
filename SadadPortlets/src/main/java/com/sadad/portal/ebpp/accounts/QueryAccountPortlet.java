...
public class QueryAccountPortlet extends SadadGenericPortlet
{
	...
	private QueryAccountSessionBean qaSesObj;
	...
	protected void doView(RenderRequest request, RenderResponse response) throws PortletException, IOException
	{
		...
		qaSesObj = (QueryAccountSessionBean) request.getPortletSession().getAttribute(PortalConstant.PORTLET_SESSION_BEAN, PortletSession.PORTLET_SCOPE);
		if (qaSesObj == null)
		{
			qaSesObj = ah.getQueryAccountObject();
			qaSesObj.getScreen().setContainer1(getJspFilePath(JSP_FOLDER, VIEW_JSP_QUERY_ACCOUNT_FORM));
			qaSesObj.getScreen().setContainer2(null);
			request.getPortletSession().setAttribute(PortalConstant.PORTLET_SESSION_BEAN, qaSesObj, PortletSession.PORTLET_SCOPE);
		}

		// Setting attributes in Request scope
		request.setAttribute(PortalConstant.PORTLET_SESSION_BEAN, qaSesObj);
	...
	public void processAction(ActionRequest request, ActionResponse response) throws PortletException, IOException
	{
		...
		if(whereTo.equals(PortalConstant.BACK))
			qaSesObj.setScreen(qaSesObj.navigate(whereTo));
	...
	public void serveResource(ResourceRequest request, ResourceResponse response) throws PortletException, IOException
	{
		...
		qaSesObj = (QueryAccountSessionBean) request.getPortletSession().getAttribute(PortalConstant.PORTLET_SESSION_BEAN, PortletSession.PORTLET_SCOPE);
		ah.setQueryAccountObject(qaSesObj);

		if (jspResourceId.equals(VIEW_JSP_QUERY_ACCOUNT_FORM))
		{}
		else if (jspResourceId.equals(VIEW_JSP_CORE_ACCOUNT_SUMMARY))
		{
			...
			qaSesObj.setPartnerKey(qaSesObj.getAccount().getBillerId());
			qaSesObj.setAccountNumber(qaSesObj.getAccount().getAccountNumber());
		}
		...
		{
			qaSesObj.setBillKey(request.getParameter("fpBillNumber"));
		}
